...
with tab_items:
    st.write("Puedes corregir descripción, cantidad, EAN13, DUN14 y la OP interna antes de generar los PDFs.")
...
with tab_preview:
    ...
    selected_bottom_code = selected_item.op_interna or DEFAULT_BOTTOM_CODE
    preview_component(po, selected_item, int(preview_index), selected_bottom_code)
    st.info(f"Vista previa: Etiqueta {int(preview_index)} de {selected_item.cantidad} para el ítem '{selected_item.descripcion}'. OP interna: {selected_bottom_code}")

    c1, c2 = st.columns([1, 1])
    with c1:
        one_pdf = generate_item_pdf(po, selected_item, selected_bottom_code)
        st.download_button(
